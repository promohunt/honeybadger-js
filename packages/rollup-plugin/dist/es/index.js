 ...
 */
function extractSourcemapDataFromBundle(outputOptions, bundle, ignorePaths) {
    if (ignorePaths === void 0) { ignorePaths = []; }
    var sourceMaps = Object.values(bundle).filter(isSourcemap);
    return sourceMaps.map(function (sourcemap) {
        return formatSourcemapData(outputOptions, sourcemap);
    }).filter(function (sourcemap) { return isNotIgnored(sourcemap, ignorePaths); });
}
function isSourcemap(file) {
    if (file.type !== 'asset' || !file.fileName.endsWith('.js.map') || !file.source) {
        return false;
    }
    var source = typeof file.source === 'string' ? file.source : file.source.toString();
    var json = JSON.parse(source);
    return !!json.sourcesContent && json.sourcesContent.length > 0;
}
function isNotIgnored(sourceMapInfo, ignorePaths) {
    var isMatch = picomatch.isMatch(sourceMapInfo.jsFilePath, ignorePaths, { basename: true });
    return !isMatch;
}
function formatSourcemapData(outputOptions, sourcemap) {
    // This fileName could include a path like 'subfolder/foo.js.map'
    var sourcemapFilename = sourcemap.fileName;
    var sourcemapFilePath = path$4.resolve(outputOptions.dir || '', sourcemapFilename);
    ...
    var jsFilename = sourcemapFilename.replace('.map', '');
    var jsFilePath = path$4.resolve(outputOptions.dir || '', jsFilename);
    return { sourcemapFilename: sourcemapFilename, sourcemapFilePath: sourcemapFilePath, jsFilename: jsFilename, jsFilePath: jsFilePath };
}
/**
...
var originalFetch = require$$0$1;
var fetchRetry = fetchRetry$1;
...
var cleanOptions_1 = cleanOptions;
var sendDeployNotification_1 = sendDeployNotification;
var uploadSourcemaps_1 = uploadSourcemaps;

function honeybadgerRollupPlugin(options) {
    var _this = this;
    var hbOptions = cleanOptions_1(options);
    return {
        name: 'honeybadger',
        writeBundle: function (outputOptions, bundle) { return __awaiter$1(_this, void 0, void 0, function () {
            var sourcemapData;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        ...
                        sourcemapData = extractSourcemapDataFromBundle(outputOptions, bundle, hbOptions.ignorePaths);
                        return [4 /*yield*/, uploadSourcemaps_1(sourcemapData, hbOptions)];
                    case 1:
                        _a.sent();
                        if (!hbOptions.deploy) return [3 /*break*/, 3];
                        return [4 /*yield*/, sendDeployNotification_1(hbOptions)];
                    case 2:
                        _a.sent();
                        _a.label = 3;
...
export { honeybadgerRollupPlugin as default };
